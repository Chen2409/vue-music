<template>
	<div id="playController" class="playController">
		...
		<!-- 歌曲详情页面 -->
		<play-music :abc="abc" :play="play" :playDetail="playlist[playCurrentIndex]" v-show="show" @back="show=!show">
		</play-music>
		<!-- 如何获取播放歌曲的mps地址 -->
		<!-- controls audio标签属性，一般不显示 -->
		<!-- audio paly（）播放音乐 pause（）暂停音乐 -->
		<audio ref="audio" v-if="playlist[playCurrentIndex]"
...
<script>
	...
	export default {
		...
		async mounted() {
			var res = await getLyirc(this.playlist[this.playCurrentIndex].id);
